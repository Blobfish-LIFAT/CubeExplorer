...
public class SessionGraph {
    ...
    public static Session resolveDimUsage(Session in, String schema){
        try {
            ...
            List<Node> nodes = doc.selectNodes("//Cube[@name='"+in.getCubeName()+"']/DimensionUsage");
            HashMap<String, String> aliases = new HashMap<>();
            nodes.stream().map(n -> (Element) n).forEach(e -> aliases.put(e.attributeValue("name"), e.attributeValue("source")));
            for (Query q : in.getQueries()){
                HashSet<QueryPart> fixed = new HashSet<>();
                for (QueryPart qp : q.getDimensions()){
                    Matcher m = dimPattern.matcher(qp.getValue());
                    if (m.matches()){
                        String dim = m.group(1);
                        if (aliases.containsKey(dim)){
                            QueryPart correct = QueryPart.newDimension(qp.getValue().replace(dim, aliases.get(dim)));
                            fixed.add(correct);
                        }else
                            fixed.add(qp);
                    }else
