 ...
 * @author Elisa
 */
public class QueryCompJaccardStructureThresWithSeveralSelectionPerLevel extends QueryComparison {

    private static HashMap<String, SelectionFragment> allLevelSelectionFragments = new HashMap();
	
	static {
		HashSet<Hierarchy> hierarchies = CubeUtils.getDefault().getHierarchies();
        for(Hierarchy h : hierarchies){
            if(h.getClass().equals(mondrian.rolap.RolapCubeHierarchy.class)){
                Member m = h.getAllMember();
                allLevelSelectionFragments.put(h.getName(), SelectionFragment.newInstance(CubeUtils.getDefault().getSelection(m.getLevel().getName(), h.getName(), m.getName())));
            }
        }
	}
    
    public QueryCompJaccardStructureThresWithSeveralSelectionPerLevel(Qfset q1, Qfset q2, double alpha, double beta, double gamma) {
    ...
    private double computeSelPredSimilarity() {
        double numberDimension = CubeUtils.getDefault().getHierarchies().size()-1;
