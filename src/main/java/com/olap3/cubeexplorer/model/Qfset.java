...
public class Qfset implements java.io.Serializable, Measurable<Qfset> {
     ...
     * @param mq the query.
     */
    public Qfset(mondrian.olap.Query mq) {
        this();
        mondrianQuery = mq;
    ...
    public ArrayList<ArrayList<ProjectionFragment>> getProjectionByHierarchy() {
        ...
        for (Hierarchy d : CubeUtils.getDefault().getHierarchies()) {
    ...
    public mondrian.olap.Query toMDX(HashSet<ProjectionFragment> projectionOnRows, HashSet<ProjectionFragment> projectionOnColumns, ArrayList<Level> levelsToLimit) {
        if (mondrianQuery == null) {
            String select;
            String where = "";
            String from = "FROM " + CubeUtils.getDefault().getCube().getName();
